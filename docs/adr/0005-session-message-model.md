...
CoActor 的消息模型从 request-response command 改为持久 Session 上的双向消息：caller 通过 `open()` 建立与 Actor 的一次会话，入站 **Action** 与出站 **Event** 均为字节负载、fire-and-forget，业务错误由 Event 表达。每个 Action 都必须携带 Session ID，所有入站都经过 Session，不提供无 Session 的单向 tell。
